<?php

  session_start();

  include_once 'includes/users.inc.php';

  $url = $_SERVER['REQUEST_URI'];


  if (strstr($url, "?")) {
    if (explode("?", $url)[1] == "check_in_db") {
      $login_email = $_POST['email'];
      $login_password = $_POST['password'];

      $email_exists = "SELECT email FROM users WHERE email = $login_email";
      $password_exists = "SELECT user_password FROM users WHERE user_password = $login_password";
      
      if ($email_exists && $password_exists) {

        $sql = "SELECT * from users WHERE email = '$login_email' and password = '$login_password';";

        $user_row = mysqli_query($conn, $sql);

        if (!$user_row) {
          echo "Couldn't retrieve user!";
        } else {
          $row_data = mysqli_fetch_row($user_row);
          $user_id = $row_data[0];


          $_SESSION['login'] = $row_data[1];

          header('Location: ' . "index.php?$user_id");
        }
      }
    }
  }

?>

<!doctype html>
<html lang="en">
  <head>
    <title>Login</title>
    <!-- Required meta tags -->
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">

    <!-- My Styles -->
    <link rel="stylesheet" href="./styles/login.css" />

    <!-- Bootstrap CSS -->
    <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css" integrity="sha384-ggOyR0iXCbMQv3Xipma34MD+dH/1fQ784/j6cY/iJTQUOhcWr7x9JvoRxT2MZw1T" crossorigin="anonymous">
  </head>
  <body>
    <div class="container-fluid">
    <div class="row">
      <div class="col-lg-7">
        <h5 id="logo"> Task <span class="second-logo-word"> Manager </span> </h5>  
        <div class="col-text">
          <h1> Sign in to account </h1>
          <hr class="primary-h-line"> </hr>
          <form action="login.php?check_in_db" method="post">
            <!-- <span id="email-label"> Email </span> -->
            <input type="email" name="email" placeholder="Email"/>
            <!-- <span id="password-label"> Password </span>  --> 
            <input type="password" name="password" placeholder="Password" />

            <div class="form-additionals d-flex justify-content-between">
              <div class="remember-me-container">
                <input type="checkbox" name="remember-me" />
                <label for="remember-me"> Remember me </label> 
              </div>
                <span class="forgot-pass"> Forgot Password? </span>
              </div>  
              <button class="sign-in" type="submit"> Sign In </button>
            </div>
          </form>
        
        <!-- In case user forgot password or wants to be remembered on the website -->
 
        
      </div>
      <div class="col-lg-5">
        <div class="col-text">
          <h1> Hello, friend! </h1>
          <hr class="primary-h-line"> </hr>
          <h5> Sign up for an account and hop on the journey of becoming more productive! </h5>
          <div class="link-container sign-up"><a href="signup.php"> Sign Up </a></div>
        </div>
      </div>
    </div>
    </div>






    <!-- Optional JavaScript -->
    <!-- jQuery first, then Popper.js, then Bootstrap JS -->
    <script src="https://code.jquery.com/jquery-3.3.1.slim.min.js" integrity="sha384-q8i/X+965DzO0rT7abK41JStQIAqVgRVzpbzo5smXKp4YfRvH+8abtTE1Pi6jizo" crossorigin="anonymous"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.7/umd/popper.min.js" integrity="sha384-UO2eT0CpHqdSJQ6hJty5KVphtPhzWj9WO1clHTMGa3JDZwrnQq4sF86dIHNDz0W1" crossorigin="anonymous"></script>
    <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/js/bootstrap.min.js" integrity="sha384-JjSmVgyd0p3pXB1rRibZUAYoIIy6OrQ6VrjIEaFf/nJGzIxFDsf4x0xIM+B07jRM" crossorigin="anonymous"></script>
  </body>
</html>